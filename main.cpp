...
#include <iostream>
using namespace std;
/*Íàïèøèòå ïðîãðàììó äëÿ ðàáîòû ñ äàòàìè ñîãëàñíî ãðèãîðèàíñêîìó êàëåíäàðþ. Â ïðîãðàììå äîëæíû ïðèñóòñòâîâàòü:
 ñòðóêòóðà õðàíÿùàÿ ãîä,ìåñÿö, ÷èñëî è óìåþùàÿ(ñ ïîìîùüþ ñîîòâåòñòâóþùåãî ìåòîäà) îïðåäåëÿòü ïî äàòå äåíü íåäåëè
ôóíêöèè îïðåäåëÿþùèå ñêîëüêî ëåò, ìåñÿöåâ è äíåé ïðîøëî ìåæäó äâóìÿ äàòàìè*/
struct calendar{
int number;
char *month;
int century;
...
if (date.year == 2|| date.year == 8|| date.year == 13|| date.year ==  19|| date.year ==  30|| date.year ==  36|| date.year ==  41|| date.year == 47|| date.year ==  58|| date.year == 64|| date.year ==  69|| date.year ==  75|| date.year ==  86|| date.year == 92|| date.year ==  97) base4 = 1;
...
if (date.century%4 == 0){
if (date.year % 4 ==0)&&(date.month == "March"|| date.month == "April"|| date.month == "May"|| date.month == "June"|| date.month == "July"|| date.month == " August"|| date.month == "September"|| date.month == "November"|| date.month == "December"||) base 2 = base 2 + 1;
}
int sum;
